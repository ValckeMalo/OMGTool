...
public class MobCreatorWindow : EditorWindow
{
    ...
    private static MobCreatorWindow window;
    ...
    {
        window = GetWindow<MobCreatorWindow>("Mob Creator");
    }
    ...
    private void Modify()
    {
        ...
        using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true)))
        {
            ...
            using (new EditorGUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.ExpandWidth(true)))
            {
                SerializedObject so = new SerializedObject(copyMob);
                SerializedProperty mobActionsProp = so.FindProperty("mobBehaviours").GetArrayElementAtIndex(CurrentIndexBehaviour).FindPropertyRelative("mobActionList");
    ...
    private void DrawFightConditon(FightCondition fightCondition, bool drawCondition)
    {
        using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.MaxWidth(WindowWidth / 2f)))
        {
            if (!drawCondition)
            {
                GUILayout.Label("Please select a condition in the first Fight Condition before set the second.", new GUIStyle(EditorStyles.boldLabel) { alignment = TextAnchor.MiddleCenter, fontSize = 30, wordWrap = true }, GUILayout.ExpandHeight(true));
                return;
            }
            ...
            if (fightCondition.ConditionType != FightConditionType.None)
            {
                    ...
                    fightCondition.ConditionType == FightConditionType.EachXTurns)
                {
                    using (new EditorGUILayout.HorizontalScope(GUILayout.ExpandWidth(true)))
                    {
                        ...
                        EditorGUILayout.FloatField(fightCondition.SpecificValue);
                    }
                }
